...
export const ChaosArenaBattle: React.FC<BattleComponentProps> = (props) => {
  ...
  // Synchronize Code Swap and Lightning Math Rewards
  useEffect(() => {
    const chaos = currentRoom.chaosEvent;
    if (!chaos) return;

    if (chaos.type === ChaosEventType.CODE_SWAP) {
      const key = `${chaos.activatedAt}`;
      if (lastHandledSwapRef.current !== key) {
        lastHandledSwapRef.current = key;
        ...
        const newCode = currentRound?.submissions?.[user?.id || '']?.code;
        if (newCode !== undefined) {
          setCode(newCode);
        }
        setSwapAlertActive(true);
        const timer = setTimeout(() => setSwapAlertActive(false), 5000);
        return () => clearTimeout(timer);
      }
    } else if (chaos.type === ChaosEventType.LIGHTNING_MATH && chaos.data?.solved) {
  ...
  }, [currentRoom.chaosEvent, currentRoom.rounds, user?.id, setCode]);
  ...
  useEffect(() => {
    ...
    socket.on('game:math_solved' as any, handleMathSolved);
    return () => {
      socket.off('game:math_solved' as any, handleMathSolved);
    };
  }, [socket]);
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-black text-neutral-200 relative select-none font-sans">
      ...
      <div className="flex lg:hidden border-b border-red-950/20 bg-black shrink-0 px-2 py-1.5 items-center justify-around text-xs font-mono safe-area-top">
        <button
      ...
      <div className="flex-1 flex overflow-hidden relative z-10">
        ...
        <div className={clsx(
          "w-full lg:w-[420px] xl:w-[440px] flex flex-col bg-[#050505] border-r border-red-950/15 shrink-0 h-full overflow-hidden",
        ...
        <div className={clsx(
          ...
          <div className="p-3 sm:p-4 border-b border-red-950/15 bg-gradient-to-b from-[#0a0202] to-black shrink-0">
            <div className="flex items-center justify-between max-w-4xl mx-auto w-full">
              
              {/* Host / Self User */}
              <div className="flex items-center space-x-2 sm:space-x-3 w-2/5">
                <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-neutral-900 border-2 border-red-900/30 flex items-center justify-center relative shadow-inner shrink-0">
                ...
                <div className="flex flex-col min-w-0">
                  <div className="flex items-center space-x-1 sm:space-x-1.5">
                    ...
                    <span className="text-[7.5px] sm:text-[8px] font-mono bg-red-500/10 text-red-400 px-1 py-0.2 rounded border border-red-500/25 shrink-0">Lvl 2</span>
                  </div>
                  <span className="text-[9px] sm:text-[10px] text-neutral-500 font-mono font-semibold mt-1 truncate">
                    {user?.rating} CP
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col items-center justify-center shrink-0 px-1.5 sm:px-2">
                <div className="text-[8px] sm:text-[9px] font-mono text-red-500/85 uppercase tracking-widest font-black mb-0.5 sm:mb-1 animate-pulse h-3 sm:h-3.5 leading-none">
                  {currentRoom.chaosEvent && new Date(currentRoom.chaosEvent.expiresAt).getTime() > Date.now()
                    ? `⚠️ ${currentRoom.chaosEvent.type.replace('_', ' ')}`
                    : 'Time Remaining'}
                </div>
                <div className="text-xl sm:text-3xl font-black font-mono tracking-wider text-red-500 leading-none shadow-[0_0_15px_rgba(239,68,68,0.15)] bg-red-950/10 border border-red-950/40 px-2.5 sm:px-4 py-1 sm:py-1.5 rounded-lg select-all">
                  {timeLeftStr}
                </div>
              </div>
              ...
              <div className="flex items-center justify-end space-x-2 sm:space-x-3 w-2/5 text-right">
                <div className="flex flex-col items-end min-w-0">
                  <div className="flex items-center space-x-1 sm:space-x-1.5">
                    <span className="text-[7.5px] sm:text-[8px] font-mono bg-red-500/10 text-red-400 px-1 py-0.2 rounded border border-red-500/25 shrink-0">Lvl 3</span>
                    <span className="text-xs font-bold text-white tracking-tight leading-none truncate max-w-[70px] sm:max-w-none">
          ...
          <div className="h-10 sm:h-11 bg-[#050505] border-b border-red-950/15 flex items-center justify-between px-3 sm:px-6 shrink-0">
            <div className="flex items-center space-x-2 sm:space-x-3">
              <div className="flex items-center space-x-1.5 sm:space-x-2 text-red-500 border-b border-red-500 pb-2.5 sm:pb-3 pt-1.5 sm:pt-2.5 text-xs font-bold font-mono">
                <Code2 className="w-3.5 h-3.5" />
                <span>solution.py</span>
              </div>
              <span className="text-[9px] text-neutral-600 font-bold font-mono tracking-widest uppercase hidden xs:block">
                Python 3
              </span>
            </div>
            ...
            <div className="flex items-center space-x-1.5 sm:space-x-2">
              <button
                ...
                className="flex items-center space-x-1 sm:space-x-1.5 px-2.5 sm:px-3 py-1 sm:py-1.5 bg-neutral-900 hover:bg-neutral-850 hover:text-white disabled:opacity-30 text-neutral-350 text-[10px] font-black uppercase rounded border border-neutral-800 transition-all active:scale-95 font-mono"
              >
                {isRunningCode ? (
              ...
              <button
                ...
                disabled={isRunningCode || isJudging || !isPlaying || isEditorFrozen}
                className="flex items-center space-x-1 sm:space-x-1.5 px-3 sm:px-4 py-1 sm:py-1.5 bg-gradient-to-r from-red-700 to-red-600 hover:from-red-600 hover:to-red-500 text-white text-[10px] font-black uppercase rounded transition-all active:scale-95 disabled:opacity-30 font-mono shadow-[0_0_15px_rgba(239,68,68,0.2)] border border-red-500/20"
              >
                {isJudging ? (
          ...
          <div className={clsx(
            "flex-1 relative border-b border-red-950/15 min-h-0",
            ...
            {/* CODE SWAP Fullscreen Anomaly Alert Overlay */}
            <AnimatePresence>
              {swapAlertActive && (
                <motion.div
                  ...
                  className="absolute inset-0 z-50 bg-black/95 backdrop-blur-lg flex flex-col items-center justify-center p-8 text-center border-2 border-amber-500 shadow-[0_0_50px_rgba(245,158,11,0.3)]"
                >
                  <motion.div 
                    animate={{ rotate: 360 }}
                    transition={{ duration: 3, repeat: Infinity, ease: 'linear' }}
                    className="w-16 h-16 bg-amber-500/20 border-2 border-amber-500/50 rounded-2xl flex items-center justify-center mb-5"
                  >
                    <RefreshCw className="w-8 h-8 text-amber-400" />
                  </motion.div>
                  <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/20 border border-amber-500/40 text-amber-300 font-mono text-xs font-black uppercase tracking-widest mb-3">
                    <AlertCircle className="w-4 h-4 text-amber-400" />
                    CHAOS ANOMALY: CODE SWAP DETECTED
                  </div>
                  ...
                  <p className="text-zinc-300 text-xs md:text-sm max-w-md mb-6 leading-relaxed">
                    Your code has been exchanged with your opponent. You are now working on your opponent's codebase. Adapt and solve!
                  </p>
                  <button
                    onClick={() => setSwapAlertActive(false)}
                    className="px-6 py-2.5 bg-amber-500 hover:bg-amber-400 text-black font-mono font-black text-xs uppercase tracking-wider rounded-lg transition-all active:scale-95 shadow-lg shadow-amber-500/20"
                  >
                    Engage New Codebase
                  </button>
                </motion.div>
            ...
            {/* LIGHTNING MATH REWARD CELEBRATION */}
            <AnimatePresence>
              {mathRewardActive && (
                <motion.div
            ...
            {/* ACTIVE CHAOS ANOMALY HUD ARRIVAL BANNERS */}
            <AnimatePresence>
              {currentRoom.chaosEvent && eventTimeLeftSec !== null && currentRoom.chaosEvent.type === ChaosEventType.FREE_DRY_RUN && (
                <motion.div
                  initial={{ y: -30, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  exit={{ y: -30, opacity: 0 }}
                  className="absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-cyan-950/90 border border-cyan-500/40 rounded-xl px-5 py-2.5 shadow-[0_0_25px_rgba(6,182,212,0.25)] backdrop-blur-md flex items-center gap-3"
                >
                  <div className="w-7 h-7 rounded-lg bg-cyan-500/20 border border-cyan-500/40 flex items-center justify-center">
                    <Zap className="w-4 h-4 text-cyan-400" />
                  </div>
                  <div>
                    <div className="text-cyan-300 font-mono font-black text-[11px] uppercase tracking-wider">
                      ⚡ Anomaly: Unlimited Free Dry Runs!
                    </div>
                    <div className="text-zinc-400 font-mono text-[9px]">
                      Test execution sandbox rate-limits bypassed for <span className="text-cyan-300 font-bold">{eventTimeLeftSec}s</span>
                    </div>
                  </div>
                </motion.div>
              )}

              {currentRoom.chaosEvent && eventTimeLeftSec !== null && currentRoom.chaosEvent.type === ChaosEventType.BONUS_ACCEPTED && (
                <motion.div
                  initial={{ y: -30, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  exit={{ y: -30, opacity: 0 }}
                  className="absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-purple-950/90 border border-purple-500/40 rounded-xl px-5 py-2.5 shadow-[0_0_25px_rgba(168,85,247,0.25)] backdrop-blur-md flex items-center gap-3"
                >
                  <div className="w-7 h-7 rounded-lg bg-purple-500/20 border border-purple-500/40 flex items-center justify-center">
                    <Sparkles className="w-4 h-4 text-purple-400" />
                  </div>
                  <div>
                    <div className="text-purple-300 font-mono font-black text-[11px] uppercase tracking-wider">
                      🌟 Critical Multiplier Active (+300 PTS)!
                    </div>
                    <div className="text-zinc-400 font-mono text-[9px]">
                      Submit passing solution before window expires in <span className="text-purple-300 font-bold">{eventTimeLeftSec}s</span>
                    </div>
                  </div>
                </motion.div>
              )}

              {currentRoom.chaosEvent && eventTimeLeftSec !== null && currentRoom.chaosEvent.type === ChaosEventType.TIME_WARP && (
                <motion.div
                  initial={{ y: -30, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  exit={{ y: -30, opacity: 0 }}
                  className="absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-indigo-950/90 border border-indigo-500/40 rounded-xl px-5 py-2.5 shadow-[0_0_25px_rgba(99,102,241,0.25)] backdrop-blur-md flex items-center gap-3"
                >
                  <div className="w-7 h-7 rounded-lg bg-indigo-500/20 border border-indigo-500/40 flex items-center justify-center">
                    <Clock className="w-4 h-4 text-indigo-400" />
                  </div>
                  <div>
                    <div className="text-indigo-300 font-mono font-black text-[11px] uppercase tracking-wider">
                      ⏳ Temporal Warp: Match Time Shifted!
                    </div>
                    <div className="text-zinc-400 font-mono text-[9px]">
                      Round duration altered by <span className="text-indigo-300 font-bold">{currentRoom.chaosEvent?.data?.delta > 0 ? '+30s' : '-30s'}</span>
                    </div>
                  </div>
                </motion.div>
              )}

              {currentRoom.chaosEvent && eventTimeLeftSec !== null && currentRoom.chaosEvent.type === ChaosEventType.OPPONENT_CODE_VIEW && (
                <motion.div
                  initial={{ y: -30, opacity: 0 }}
                  animate={{ y: 0, opacity: 1 }}
                  exit={{ y: -30, opacity: 0 }}
                  className="absolute top-4 left-1/2 -translate-x-1/2 z-40 bg-emerald-950/90 border border-emerald-500/40 rounded-xl px-5 py-2.5 shadow-[0_0_25px_rgba(16,185,129,0.25)] backdrop-blur-md flex items-center gap-3"
                >
                  <div className="w-7 h-7 rounded-lg bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center">
                    <Eye className="w-4 h-4 text-emerald-400" />
                  </div>
                  <div>
                    <div className="text-emerald-300 font-mono font-black text-[11px] uppercase tracking-wider">
                      👁️ Recon Radar: Opponent Code Intercept!
                    </div>
                    <div className="text-zinc-400 font-mono text-[9px]">
                      Opponent code telemetry decrypted for <span className="text-emerald-300 font-bold">{eventTimeLeftSec}s</span>
                    </div>
                  </div>
                </motion.div>
              )}
        ...
        <div className="w-72 hidden xl:flex flex-col bg-[#050505] border-l border-red-950/15 flex-shrink-0">
